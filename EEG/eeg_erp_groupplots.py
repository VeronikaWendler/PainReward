...
#-----------------------------------------------------------------------------------------------------
# Multivariate Regression Plots from MP without looping over regressors 
...
for tidx, timepos in enumerate(times_pos):
    ...
    if tidx+1 == len(plot_times):
        ...
        fig.savefig(opj(outfigpath, 'fig_topo_beta_cbar_' + regvar + '.svg'),
...
for c in chan_to_plot:
    ...
    for idx, bin in enumerate([str(i + 1) for i in range(nbins)]):
        line_axis.plot(all_epos[0].times * 1000, 
                       ...
                       label=str(idx + 1), linewidth=2, 
                       color=plt.cm.get_cmap(cmap, nbins)(idx / nbins))
