...
for p in part:
    ...
    if version == 1:      
        if lock_type == 'cue':
            ...
            if not os.path.exists(outdir):
                os.mkdir(outdir)
            print("version 1: outdir = opj(outpath,  p, 'eeg', 'erps_resp')")
        else:
    ...
    if version == 1:
        ...
        for cond in events_id.keys():
            ...
            # save evoked
            if lock_type == 'cue':
                fname = f"{p}_decision_{cond}_ave.fif"        # cond == 'off+'
            else:
                fname = f"{p}_decision_resp_{cond}_ave.fif"   # cond == 'resp_any'
        ...
        if lock_type == 'cue':
            ...
            report_name = p + '_decision_erps_report.html'
        else:
            section = 'ERPs for responses'
            title = 'Butterfly plots for responses'
            report_name = p + '_decision_resp_erps_report.html'
    ...
    # Save single-trial epochs
    if version == 1:
        if lock_type == 'cue':
            fname = p + '_decision_cues_singletrials-epo.fif'
            print("erp_cues_single.save(opj(outdir,_decision_cues_singletrials-epo.fif')")
        elif lock_type == 'response':
            fname = p + '_decision_resp_singletrials-epo.fif'
            print("erp_cues_single.save(opj(outdir,_decision_resp_singletrials-epo.fif')")
        erp_cues_single.save(opj(outdir, fname), overwrite=True)
    
    elif version == 2:
        erp_cues_single.save(opj(outdir, p
                                 + '_passive_cues_singletrials-epo.fif'),
                             overwrite=True)
        print("erp_cues_single.save(opj(outdir,p_passive_cues_singletrials-epo.fif')")
...
if version == 1 and lock_type == 'cue':
    ...
    reject_stats.to_csv(opj(outpath,
                            'passive_erps_rejectionstats.csv'))          
    reject_stats.describe().to_csv(opj(outpath,
                                       'passive_'
                                       + 'erps_rejectionstats_desc.csv')) 
    print("reject_stats.to_csv(opj(outpath,passive_erps_rejectionstats.csv'))")
...
if version == 1 and lock_type == 'response':
    amp_lat = [[-0.5, 0.1]]
    print("amp_lat = -0.5 -> 0.1")
else:
    amp_lat = [[0.4, 0.8]] 
    print("amp_lat = 0.4 -> 0.8")
...
if version == 1:
    ...
    for p in part:
        outdir = opj(outpath,  p, 'eeg', 'erps_passive')                               
        epo = mne.read_epochs(opj(outdir, p + '_passive_cues_singletrials-epo.fif'))
    ...
    print("all_meta.to_csv(opj(outpath, 'passive_erpsmeta.csv'), index=False)")   
