...
# @ Date: 2024
# @ Description: Passive phase EEG mass-univariate analysis — end-to-end pipeline
#
# Sections:
...
def summarize_time_windows(sig_mask, times_ms):
    """sig_mask: (time, channels) → list of window dicts."""
...
def peak_stat_in_window(stat_map, sig_mask, s, e, ch_names, times_ms):
    """Find peak absolute stat within a time window; stat_map: (time, channels)."""
...
def summarize_beta_map(beta_data, sig_mask, times_ms, ch_names):
    """beta_data: (channels, time); sig_mask: (time, channels)."""
...
# ===========================================================
# SECTION 1: PASSIVE FIRST-LEVEL REGRESSION
# ===========================================================
...
def find_passive_beh(pa: str) -> Path:
    # Check derivatives first, consistent with find_passive_epochs
    cand1 = basepath / "derivatives" / pa / "eeg" / f"{pa}_task-passive_beh.tsv"
    cand2 = basepath / pa / "eeg" / f"{pa}_task-passive_beh.tsv"
    return cand1 if cand1.exists() else cand2
...
def load_passive_beh_with_trialsnum(beh_path: Path) -> pd.DataFrame:
    ...
    if "condition" not in beh.columns or "level" not in beh.columns:
        raise ValueError(
            f"Passive beh file missing 'condition'/'level'. Columns: {list(beh.columns)}"
        )
    beh["condition"] = beh["condition"].astype(str).str.lower().str.strip()
    beh["level"] = pd.to_numeric(beh["level"], errors="coerce")
    return beh[beh["condition"].isin(["p", "m"]) & np.isfinite(beh["level"])].copy()


def merge_beh_into_epochs_on_trialsnum(
    epo: mne.Epochs, beh: pd.DataFrame, pa: str
) -> mne.Epochs:
    md = pd.DataFrame({"trialsnum": np.arange(1, len(epo) + 1)}) \
        if epo.metadata is None \
        else epo.metadata.reset_index(drop=True).copy()
    ...
    merged = md.merge(beh[["trialsnum", "condition", "level"]],
                      on="trialsnum", how="left", validate="1:1")
    ...
    if merged["condition"].isna().any() or merged["level"].isna().any():
        ...
        raise ValueError(
            ...
            "Epochs order and beh.tsv order may differ for this subject."
        )
...
print(f"\nPASSIVE: subjects with epochs + beh: {len(part_passive)}")

for pa in part_passive:
    print(f"\n--- Processing {pa} ---")
    try:
        epo = mne.read_epochs(str(find_passive_epochs(pa)), preload=True)
        beh = load_passive_beh_with_trialsnum(find_passive_beh(pa))
        ...
        if len(epo) < 20:
            print(f"  Skipping: too few epochs after artifact rejection (n={len(epo)})")
        ...
        if is_pain.sum() < 5 or is_money.sum() < 5:
            print(f"  Skipping: too few trials (pain={is_pain.sum()}, money={is_money.sum()})")
            skipped_subjects.append(pa)
            continue

        if np.nanstd(level[is_pain]) == 0 or np.nanstd(level[is_money]) == 0:
            print(f"  Skipping: zero variance in level within a condition")
            skipped_subjects.append(pa)
            continue
        ...
        if not np.all(np.isfinite(design.to_numpy())):
            print(f"  Skipping: NaN/Inf in design matrix")
            skipped_subjects.append(pa)
            continue
        ...
        print(f"  Included (n_epochs={len(epo)})")
...
# Save a dataframe with the included and skipped subjects for record-keeping
subj_df = pd.DataFrame({
    "participant_id": included_subjects + skipped_subjects,
    "included_in_passive_analysis": [True] * len(included_subjects) + [False] * len(skipped_subjects)
})
...
for map_name in all_maps:
    ...
    for wi, w in enumerate(windows, start=1):
        ...
        if map_name in beta_data_lookup:
            bdata_t = beta_data_lookup[map_name].T  # (time, ch)
